...
# model = simple_attn(32, 4).to(device)
model = Attention(32,4).to(device)
input = torch.randn(1, 100*100, 32).to(device)
flops, params = profile(model, inputs=(input,100,100))
# gpu_tracker.track()  #开始检测
torch.cuda.synchronize()
time_start = time.time()
out = model(input,100,100).to(device)
torch.cuda.synchronize()
time_end = time.time()
